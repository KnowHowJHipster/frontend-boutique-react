...
describe('CustomerDetails e2e test', () => {
  ...
  // const customerDetailsSample = {"gender":"FEMALE","phone":"387-469-9274","addressLine1":"rosy what after","city":"Port Sonnyworth","country":"Bulgaria"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"GjfRGa@HyEAkD\\6EkBvjz\\:LRU82\\;RClhj\\<QZLHr\\QLAdTPW","firstName":"Russell","lastName":"Runte","email":"Jaunita_Stoltenberg@yahoo.com","imageUrl":"quizzically helpful","langKey":"telecommut"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new CustomerDetails page', () => {
    ...
    it.skip('should create an instance of CustomerDetails', () => {
      ...
      cy.get(`[data-cy="phone"]`).type('(819) 531-0580 x18595');
      cy.get(`[data-cy="phone"]`).should('have.value', '(819) 531-0580 x18595');

      cy.get(`[data-cy="addressLine1"]`).type('persuade');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'persuade');

      cy.get(`[data-cy="addressLine2"]`).type('oh scrabble');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'oh scrabble');

      cy.get(`[data-cy="city"]`).type('Douglasworth');
      cy.get(`[data-cy="city"]`).should('have.value', 'Douglasworth');

      cy.get(`[data-cy="country"]`).type('Qatar');
      cy.get(`[data-cy="country"]`).should('have.value', 'Qatar');
