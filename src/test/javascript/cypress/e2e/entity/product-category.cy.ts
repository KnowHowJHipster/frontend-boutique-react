...
describe('ProductCategory e2e test', () => {
  ...
  const productCategorySample = { name: 'after whose' };
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/product-categories?page=0&size=20>; rel="last",<http://localhost/api/product-categories?page=0&size=20>; rel="first"',
              },
  ...
  describe('new ProductCategory page', () => {
    ...
    it('should create an instance of ProductCategory', () => {
      cy.get(`[data-cy="name"]`).type('geez as');
      cy.get(`[data-cy="name"]`).should('have.value', 'geez as');

      cy.get(`[data-cy="description"]`).type('counter');
      cy.get(`[data-cy="description"]`).should('have.value', 'counter');
